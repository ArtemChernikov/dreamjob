...
class VacancyControllerTest {
    ...
    public void whenRequestVacancyByIdThenGetPageWithVacancy() {
        var searchId = 1;
        var expectedVacancy = new Vacancy(searchId, "test1", "desc1", now(), true, 1, 2);
        when(vacancyService.findById(searchId)).thenReturn(Optional.of(expectedVacancy));

        var model = new ConcurrentModel();
        var view = vacancyController.getById(model, searchId);
    ...
    @Test
    public void whenRequestVacancyByNonExistIdThenGetPageWithError() {
        var nonExistId = 500;
        var expectedMessage = "Вакансия с указанным идентификатором не найдена";
        when(vacancyService.findById(nonExistId)).thenReturn(Optional.empty());

        var model = new ConcurrentModel();
        var view = vacancyController.getById(model, nonExistId);
    ...
    @Test
    public void whenPostUpdateVacancyThenGetPageWithVacancies() throws IOException {
        var updatedVacancy = new Vacancy(1, "updatedName", "updatedDesc", now(), true, 2, 2);
        var fileDto = new FileDto(testFile.getOriginalFilename(), testFile.getBytes());
        var vacancyArgumentCaptor = ArgumentCaptor.forClass(Vacancy.class);
        var fileDtoArgumentCaptor = ArgumentCaptor.forClass(FileDto.class);
    ...
    @Test
    public void whenPostUpdateVacancyByNonExistIdThenGetPageWithError() {
        var updatedVacancy = new Vacancy(1, "updatedName", "updatedDesc", now(), true, 2, 2);
        var expectedMessage = "Вакансия с указанным идентификатором не найдена";
    ...
    @Test
    public void whenPostUpdateVacancyThenGetPageWithError() {
        var expectedException = new RuntimeException("Could not update vacancy");
        var updatedVacancy = new Vacancy(1, "updatedName", "updatedDesc", now(), true, 2, 2);
    ...
    @Test
    public void whenRequestDeleteVacancyByIdThenGetPageWithVacancies() {
    ...
    @Test
    public void whenRequestDeleteVacancyByIdThenGetPageWithError() {
        var expectedMessage = "Вакансия с указанным идентификатором не найдена";
